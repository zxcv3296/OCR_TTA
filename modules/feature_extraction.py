...
class ResNet_FeatureExtractor(nn.Module):
    ...
    def __init__(self, input_channel, output_channel=512):
        ...
        modules = list(m.children())[:-2]  # 마지막 avgpool, fc 제거
        self.ConvNet = nn.Sequential(*modules)


        if input_channel != 3:
            self.conv1 = nn.Conv2d(in_channels=input_channel, out_channels=3, kernel_size=1, stride=1, padding=0, bias=False)
            self.bn1 = nn.BatchNorm2d(3)
            self.ReLU1 = nn.ReLU()

        self.conv_fin = nn.Conv2d(in_channels=2048, out_channels=output_channel, kernel_size=1, stride=1, padding=0, bias=False)
        self.bn_fin = nn.BatchNorm2d(output_channel)
    ...
    def forward(self, input):
        ...
        feat=self.ConvNet(feat)
        feat = self.conv_fin(feat)
        feat = self.bn_fin(feat)
        feat = self.ReLU_fin(feat)
        return feat
